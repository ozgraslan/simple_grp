...
class SimpleGRP(nn.Module):
    def __init__(self, *, state_dim, action_dim, dim, depth, heads, mlp_dim, image_dim = 0, num_image_tokens=0, 
        ...
        self.to_patch_embedding = nn.Linear(image_dim, dim)
...
def init_model(transformer_params = {"depth": 6, "heads": 8, "mlp_dim": 2048}, 
    ...
    model = SimpleGRP(
        ...
        image_dim = image_params["image_dim"],
        num_image_tokens = image_params["num_image_tokens"],
...
def evaluate_model(task_suite_name="libero_10", num_trials=5, img_shape=(256, 256, 3)):
    ...
    for task_id in range(task_suite.n_tasks):
        ...
        task_attention_mask = get_att_mask(block_mask_arr.unsqueeze(0).repeat(task_text_masks.shape[0], 1), task_text_masks).unsqueeze(1).repeat(1, transformer_heads, 1, 1)
        ...
        for _ in range(num_trials):
            ...
            if use_wandb:
                ...
                wandb.log({"video": wandb.Video(np.transpose(frames, axes=(0,3,1,2)), caption=task_description, fps=dataset_metadata.fps)})
...
if __name__ == "__main__":
    ...
    model_path = "/network/projects/real-g-grp/simple_grp/simple_grp_dino_base_patches2_"
    ...
    if not do_eval_on_env:
        train_dataset = torch.cat([
                                   TensorDict.load_memmap("/network/projects/real-g-grp/libero_td/libero10_dinov2_base_patch_dataset_488.pt"), 
                                   TensorDict.load_memmap("/network/projects/real-g-grp/libero_td/libero10_dinov2_base_patch_dataset_791.pt")
                                ], dim=0).to(device)
    ...
    vis_config = AutoConfig.from_pretrained(dino_model_name, torch_dtype=dtype)
    vis_processor = AutoProcessor.from_pretrained(dino_model_name, out_features=[], torch_dtype=dtype)
    ...
    attention_mask = get_att_mask(block_mask_arr.unsqueeze(0).repeat(text_masks.shape[0], 1), text_masks).unsqueeze(1).repeat(1, transformer_heads, 1, 1)
    ...
    model = init_model(**model_params).to(device=device, dtype=dtype)
    print(model)
    if do_eval_on_env:
        model.load_state_dict(torch.load(model_path))
        optimizer = None
        loss_function = None
        vis_backbone = AutoBackbone.from_pretrained(dino_model_name, out_features=[], torch_dtype=dtype).to(device)
    ...
    if do_eval_on_env:
        evaluate_model(img_shape= ds_features["observation.images.image"]["shape"] )
    else:
        step = 0
        done = False
        while not done:
        ...
        torch.save(model.state_dict(), model_path + "_final.pt")
